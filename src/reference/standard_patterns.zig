...
test "GPA" {
    ...
    defer {
        ...
        // Fail test; can't try in defer as defer is executed
        // after we return.
    }
...
test "createFile, write, seekTo, read" {
    ...
    defer {
        file.close();
        std.fs.cwd().deleteFile(file_name) catch @panic("TEST FAIL");
    }
...
test "file stat" {
    const file_name = "junk_file2.txt";
    const file = try std.fs.cwd().createFile(
        file_name,
        .{ .read = true },
    );
